...
# База данных, полностью повторяющая логику вкладок Pocket Option
ASSETS_DATA = {
    "ru": {
        "[ВСЕ АКТИВЫ] — OTC ЦИКЛ": {
    ...
    <head>
        ...
        <style>
            ...
            .btn-vip-top { 
                ...
                color: #000 !important; font-weight: 900; font-size: 11px; cursor: pointer;

box-shadow: 0 2px 10px rgba(255,215,0,0.3); text-transform: uppercase; letter-spacing: 0.5px;
            }
            ...
            .count-btn {
                display: flex; align-items: center; gap: 8px; background: #0f131e; 
                padding: 10px 16px; border-radius: 12px; border: 1px solid #1a2233;
                cursor: pointer; font-weight: 800; font-size: 13px; transition: all 0.2s;
            }
    ...
    <div style="max-width:430px; margin:15px auto; padding:0 15px; display:flex; justify-content:space-between; align-items:center;">
        ...
        <div>
            <a href="https://t.me/+WB89-UHgktU0YmQy" target="_blank" style="text-decoration: none;">
                <button id="vip_btn_text" class="btn-vip-top">👑 VIP СИГНАЛЫ</button>
            </a>
        </div>
    </div>
    ...
    <div style="max-width:430px; margin:0 auto 30px auto; padding:25px; background:#080a10; border-radius:28px; border: 1px solid #121722; box-shadow: 0 25px 50px rgba(0,0,0,0.8); text-align:center;">
        ...
        <div id="sub_cat_block" style="text-align:left; margin-bottom:14px;">
            <label id="lbl_type">ТИП АКТИВА</label>
...
<button id="runBtn" class="btn btn-main" onclick="getLiveSignal(false)">СКАНИРОВАТЬ РЫНОК</button>
        ...
        <div class="counter-box">
            <button class="count-btn" onclick="adjustCounter('win', 1)" oncontextmenu="adjustCounter('win', -1); return false;">
                <span style="color:#586988; font-size:11px;">PROFIT +</span>
                <span id="win_counter" style="color:#00ff66;">0</span>
            </button>
            <button class="count-btn" onclick="adjustCounter('loss', 1)" oncontextmenu="adjustCounter('loss', -1); return false;">
                <span style="color:#586988; font-size:11px;">LOSS -</span>
    ...
    <script>
        ...
        const tf_options = {
            ru: ["5 сек", "15 сек", "30 сек", "1 мин", "2 мин", "3 мин", "4 мин", "5 мин", "10 мин"],
            en: ["5 sec", "15 sec", "30 sec", "1 min", "2 min", "3 min", "4 min", "5 min", "10 min"],
            ua: ["5 сек", "15 сек", "30 сек", "1 хв", "2 хв", "3 хв", "4 хв", "5 хв", "10 хв"],
            es: ["5 seg", "15 seg", "30 seg", "1 min", "2 min", "3 min", "4 min", "5 min", "10 min"],
            de: ["5 Sek", "15 Sek", "30 Sek", "1 Min", "2 Min", "3 Min", "4 Min", "5 Min", "10 Min"]
        };
        ...
        const dictionary = {
            ru: {
                title: "AI QUANTUM ENGINE ACTIVE", market: "КАТЕГОРИЯ РЫНКА", type: "ТИП АКТИВА", asset: "АКТИВНАЯ ПАРА", tf: "ИНТЕРВАЛ СВЕЧИ",
                exp: "ЭКСПИРАЦИЯ", scan: "СКАНИРОВАТЬ РЫНОК", auto: "ИИ СДЕЛАТЬ ЗА ВАС", pocket: "ОТКРЫТЬ POCKET OPTION", support: "РАЗРАБОТЧИК / SUPPORT",
                ready: "СИСТЕМА СИНХРОНИЗИРОВАНА", load: "[ АНАЛИЗ РЫНОЧНОГО ОРДЕРА... ]", UP: "ВВЕРХ / CALL", DOWN: "ВНИЗ / PUT",
                otc: "СИСТЕМА ИИ: РАСЧЕТ УСПЕШЕН", buy: "АНАЛИЗ TV: СИГНАЛ НА ПОКУПКУ", sell: "АНАЛИЗ TV: СИГНАЛ НА ПРОДАЖУ",
...
ai_search: "[ ИИ ИЩЕТ ЛУЧШУЮ ТОЧКУ ВХОДУ... ]", vip: "👑 VIP СИГНАЛЫ"
            },
            en: {
                title: "AI QUANTUM ENGINE ACTIVE", market: "MARKET CATEGORY", type: "ASSET TYPE", asset: "ACTIVE PAIR", tf: "CANDLE TIMEFRAME",
                ...
                otc: "AI SYSTEM: CALCULATION SUCCESS", buy: "TV ANALYSIS: BUY SIGNAL", sell: "TV ANALYSIS: SELL SIGNAL",
                ...
                ai_search: "[ AI SEARCHING BEST ENTRY POINT... ]", vip: "👑 VIP SIGNALS"
            },
            ua: {
                title: "AI QUANTUM ENGINE ACTIVE", market: "КАТЕГОРІЯ РИНКУ", type: "ТИП АКТИВУ", asset: "АКТИВНА ПАРА", tf: "ІНТЕРВАЛ СВІЧКИ",
                exp: "ЕКСПІРАЦІЯ", scan: "СКАНУВАТИ РИНОК", auto: "ШІ ЗРОБИТЬ ЗА ВАС", pocket: "ВІДКРИТИ POCKET OPTION", support: "РОЗРОБНИК / SUPPORT",
                ready: "СИСТЕМА СИНХРОНІЗОВАНА", load: "[ АНАЛІЗ РИНКОВОГО ОРДЕРУ... ]", UP: "ВГОРУ / CALL", DOWN: "ВНИЗ / PUT",
                otc: "СИСТЕМА ШІ: РОЗРАХУНОК УСПІШНИЙ", buy: "АНАЛІЗ TV: СИГНАЛ НА ПОКУПКУ", sell: "АНАЛІЗ TV: СИГНАЛ НА ПРОДАЖУ",
                flat: "МАТРИЦЯ ОБ'ЄМІВ: ІМПУЛЬС ТРЕНДУ", backup: "АВТОНОМНИЙ РЕЖИМ ШІ", mart_status: "[ РОЗРАХУНОК КРОКУ МАРТИНГЕЙЛУ... ]",
                mart_btn: "АКТИВУВАТИ ПЕРЕКРИТТЯ", wait: "ВХІД У УГОДУ ЧЕРЕЗ: ", process: "УГОДА В ПРОЦЕСІ ТОРГІВЛІ: ", end: "ТОРГОВИЙ ЦИКЛ ЗАВЕРШЕНО",
                ai_search: "[ ШІ ШУКАЄ КРАЩУ ТОЧКУ ВХОДУ... ]", vip: "👑 VIP СИГНАЛИ"
            },
            es: {
                title: "AI QUANTUM ENGINE ACTIVE", market: "CATEGORÍA DE MERCADO", type: "TIPO DE ACTIVO", asset: "PAR ACTIVO", tf: "TEMPORALIDAD VELA",
                exp: "TIEMPO EXPIRACIÓN", scan: "ESCANEAR MERCADO", auto: "IA HACER POR TI", pocket: "ABRIR POCKET OPTION", support: "DESARROLLADOR / SUPPORT",
                ready: "SISTEMA SINCRONIZADO", load: "[ ANALIZANDO ORDEN DE MERCADO... ]", UP: "SUBIR / CALL", DOWN: "BAJAR / PUT",
                otc: "SISTEMA IA: CÁLCULO EXITOSO", buy: "ANÁLISIS TV: SEÑAL DE COMPRA", sell: "ANÁLISIS TV: SEÑAL DE VENTA",
                flat: "MATRIZ DE VOLUMEN: IMPULSO DE TENDENCIA", backup: "MODO IA AUTÓNOMO", mart_status: "[ CALCULANDO PASO MARTINGALA... ]",
                mart_btn: "ACTIVAR MULTIPLICADOR", wait: "ENTRAR EN OPERACIÓN EN: ", process: "OPERACIÓN EN CURSO: ", end: "CICLO DE TRADING COMPLETADO",
                ai_search: "[ IA BUSCANDO MEJOR PUNTO DE ENTRADA... ]", vip: "👑 SEÑALES VIP"
            },
            de: {
                title: "AI QUANTUM ENGINE ACTIVE", market: "MARKTKATEGORIE", type: "ASSET-TYP", asset: "AKTIVES PAAR", tf: "KERZEN TIMEFRAME",
                exp: "ABLAUFZEIT", scan: "MARKT SCANNEN", auto: "KI FÜR DICH TUN", pocket: "POCKET OPTION ÖFFNEN", support: "ENTWICKLER / SUPPORT",
                ready: "SYSTEM SYNCHRONISIERT", load: "[ MARKTORDER WIRD ANALYSIERT... ]", UP: "CALL", DOWN: "PUT",
                otc: "KI-SYSTEM: BERECHNUNG ERFOLGREICH", buy: "TV-ANALYSE: KAUFSIGNAL", sell: "TV-ANALYSE: VERKAUFSIGNAL",
                flat: "VOLUMENMATRIX: TRENDIMPULS", backup: "AUTONOMER KI-MODUS", mart_status: "[ MARTINGALE-SCHRITT WIRD BERECHNET... ]",
                mart_btn: "MULTIPLIER AKTIVIEREN", wait: "HANDEL STARTET IN: ", process: "HANDEL LÄUFT: ", end: "HANDELSZYKLUS BEENDET",
                ai_search: "[ KI SUCHT NACH BESTEM EINSTIEGSPUNKT... ]", vip: "👑 VIP-SIGNALE"
            }
        };
...
function changeLang() {
            ...
            let oldCatIdx = Math.max(0, document.getElementById('cat').selectedIndex);
            
            let catSelect = document.getElementById('cat');
            catSelect.innerHTML = "";
            Object.keys(rawData[l]).forEach(c => { catSelect.innerHTML += <option>${c}</option>; });
            catSelect.selectedIndex = oldCatIdx <= catSelect.options.length - 1 ? oldCatIdx : 0;
        ...
        function init(){
            let l = document.getElementById('lang').value;
            Object.keys(rawData[l]).forEach(c => document.getElementById('cat').innerHTML += <option>${c}</option>);
            updCategory();
        }
        ...
        function adjustCounter(type, amount) {
            if(type === 'win') {
                ...
                losses = Math.max(0, losses + amount);
        ...
        function calcLocalPayout(assetName) {
            ...
            if(anyCrypto(assetName)) return 78;
            return 82;
        }
        
        function anyCrypto(asset) {
            return ["BTC", "ETH", "SOL", "XRP", "LTC", "TRX", "BNB", "DOGE"].some(c => asset.includes(c));
        }

        function updCategory(isLangChange = false){
            let l = document.getElementById('lang').value;
            let c = document.getElementById('cat').value;
            
            let oldSubIdx = Math.max(0, document.getElementById('sub_cat').selectedIndex);
            let oldAssetIdx = Math.max(0, document.getElementById('asset').selectedIndex);

            let types = Object.keys(rawData[l][c]);
            document.getElementById('sub_cat').innerHTML = types.map(t => <option>${t}</option>).join('');
            
            if(isLangChange) {
                document.getElementById('sub_cat').selectedIndex = oldSubIdx <= types.length - 1 ? oldSubIdx : 0;
            }
            updSubCategory(isLangChange);
        }

        function updSubCategory(isLangChange = false) {
            let l = document.getElementById('lang').value;
            let c = document.getElementById('cat').value;
            let t = document.getElementById('sub_cat').value;
            let oldAssetIdx = Math.max(0, document.getElementById('asset').selectedIndex);
            
            let currentAssets = rawData[l][c][t] || [];
            document.getElementById('asset').innerHTML = currentAssets.map(a => <option>${a}</option>).join('');
            
            if(isLangChange) {
                document.getElementById('asset').selectedIndex = oldAssetIdx <= document.getElementById('asset').options.length - 1 ? oldAssetIdx : 0;
            }
            updAsset(isLangChange);
        }

function updAsset(isLangChange = false) {
            let l = document.getElementById('lang').value;
            ...
            let oldTfIdx = Math.max(0, document.getElementById('time').selectedIndex);
            let oldExpIdx = Math.max(0, document.getElementById('exp').selectedIndex);

            let tfSelect = document.getElementById('time');
            tfSelect.innerHTML = "";
            tf_options[l].forEach(o => { tfSelect.innerHTML += <option>${o}</option>; });
            tfSelect.selectedIndex = oldTfIdx;

            let expSelect = document.getElementById('exp');
            expSelect.innerHTML = "";
            let isOtc = asset.includes("OTC");
            
            let currentOptions = isOtc ? tf_options[l] : tf_options[l].slice(3); 
            currentOptions.forEach(o => { expSelect.innerHTML += <option value="${o}">${o}</option>; });
            expSelect.selectedIndex = oldExpIdx <= expSelect.options.length - 1 ? oldExpIdx : 0;
        }
        ...
        async function aiDoForYou() {
            ...
            let subCats = Object.keys(rawData[l][randomCat]);
        ...
        function parseToSeconds(valStr) {
            ...
            return num * (valStr.includes("мин")  valStr.includes("min")  valStr.includes("хв") || valStr.includes("Min") ? 60 : 1);
        }
