    ...
    <div style="max-width:430px; margin:20px auto; padding:25px; background:#080a10; border-radius:28px; border:1px solid #1a2233; text-align:center;">
        ...
        <div style="display:flex; justify-content:flex-end;">
            <a href="https://t.me/+WB89-UHgktU0YmQy" target="_blank" style="text-decoration:none;">
                <button style="padding:8px 15px; background:linear-gradient(45deg, #ffd700, #ff8c00); border:none; border-radius:8px; color:#000; font-weight:900; font-size:12px; cursor:pointer;">VIP</button>
            </a>
        </div>
        ...
        <div style="margin-top:30px;">
            <a href="https://t.me/SupportDev_Bot" target="_blank" style="color:#556688; text-decoration:none; font-size:12px;">🛠 СВЯЗЬ С РАЗРАБОТЧИКОМ</a>
        </div>
    </div>
    <script>
        ...
        async function startProcess(isAI) {{
            ...
            timer = setInterval(() => {{
                ...
                if(s-- <= 0) {{
                    clearInterval(timer);
                    document.getElementById('timer').innerText = "ИДЕТ СДЕЛКА (ЭКСПИРАЦИЯ 60с)...";
                    let e = 60;
                    let exp = setInterval(() => {{
                        document.getElementById('timer').innerText = "ДО КОНЦА: " + e;
                        if(e-- <= 0) {{ 
                            clearInterval(exp); 
                            document.getElementById('timer').innerText = "СДЕЛКА ЗАКРЫТА"; 
